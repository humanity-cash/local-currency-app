...
import { CreditCardDetailsErrors } from "../../utils/types";
...
export const CreditCardForm = (props: CreditCardFormProps) => {
	...
	const [validationErrors, setValidationErrors] = useState<CreditCardDetailsErrors>({});
	...
	return (
		<View>
			...
			{showValidation && validationErrors.number && (<Text h3 style={{ marginTop: 5, color: colors.textError}}>{validationErrors.number}</Text>)}
			<BlockInput
				name="number"
			...
			<View style={{ flexDirection: 'row' }}>
				<View style={{ flex: 1 }}>
					...
					{showValidation && validationErrors.date && (<Text h3 style={{ marginTop: 5, color: colors.textError}}>{validationErrors.date}</Text>)}
					<View style={{ flexDirection: 'row' }}>
						<BlockInput
							name="expireMonth"
							placeholder="MM"
				...
				<View style={{ flex: 1 }}>
					...
					{showValidation && validationErrors.cvc && (<Text h3 style={{ marginTop: 5, color: colors.textError}}>{validationErrors.cvc}</Text>)}
					<BlockInput
						name="cvc"
						placeholder="123"
						keyboardType="number-pad"
