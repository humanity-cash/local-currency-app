...
const TransactionDetails = (props: TransactionDetailsProps) => {
  const renderTitle = (transaction: Transaction) => {
    switch (transaction.type) {
      ...
      case TransactionType.SELL:
        ...
        } ${(transaction as OrderTransaction).title}`;
      case TransactionType.BUY:
        return `Purchased ${(transaction as OrderTransaction).shares} ${
          (transaction as OrderTransaction).shares === 1 ? "share" : "shares"
        } ${(transaction as OrderTransaction).title}`;
      case TransactionType.RESERVATION:
        return `Bid order ${(transaction as OrderTransaction).shares} ${
          (transaction as OrderTransaction).shares === 1 ? "share" : "shares"
        } ${(transaction as OrderTransaction).title}`;
    }
  };
  ...
  const renderPrice = (transaction: Transaction) => {
    const shares =
      transaction && "shares" in transaction ? transaction.shares : 1;
    switch (transaction.type) {
      case TransactionType.ADDCASH:
      case TransactionType.SELL:
      ...
      case TransactionType.WITHDRAW:
        return (
          <View style={styles.transferView}>
            <Text h3>
              {transaction.type === TransactionType.ADDCASH ? "From" : "To"}{" "}
              bank account: NL 12 ABCD 1234 5678 90
            </Text>
          </View>
        );
      case TransactionType.SELL:
          ...
          <View style={styles.detailsView}>
            <View style={styles.gridView}>
              ...
              <Text style={styles.boldText}>
                {(transaction as OrderTransaction).shares} shares
              </Text>
            </View>
            <View style={styles.gridView}>
              <Text h3>
                {transaction.type === TransactionType.SELL ? "ask" : "bid"} per
                share
              </Text>
              <Text style={styles.boldText}>
            ...
            <View style={styles.gridView}>
              <Text h3>
                Total{" "}
                {transaction.type === TransactionType.SELL ? "ask" : "bid"}{" "}
                price
              </Text>
              <Text style={styles.boldText}>
                {formatValue(
                  transaction.price * (transaction as OrderTransaction).shares
                )}
              </Text>
            </View>
          </View>
