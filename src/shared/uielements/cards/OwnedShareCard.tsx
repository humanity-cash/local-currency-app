...
const OwnedShareCard = (props: ShareProps) => {
  ...
  const diffColor =
    percentage.growth === "raise" ? colors.textSuccess : colors.textWarning;
  return (
        ...
        <View style={styles.gridWiderView}>
          ...
          <View style={{ flexDirection: "row" }}>
            <Text style={styles.boldText}>
              {formatValue(marketEntry.price)}
            </Text>
            <Text
              style={{ ...styles.boldText, color: diffColor, marginLeft: 10 }}
            >
              {percentage.growth === "raise" ? "+" : "-"}
              {percentage.value}%
            </Text>
          </View>
        </View>
