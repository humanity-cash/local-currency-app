...
const ShareCardView = (props: ShareProps) => {
  const { share } = props;
  const color =
    share.diff === Growth.INCREASE ? colors.textSuccess : colors.textWarning;
  return (
    <TouchableWithoutFeedback
      onPress={() => props.onClick(share.marketEntryId)}
    >
      <View style={styles.mainView}>
        <View style={styles.headerView}>
          <Text style={styles.headerName}>{share.name}</Text>
          <Text style={{ ...styles.headerChangeRate, color }}>
            {share.changeRate}%
          </Text>
        </View>
        <View style={styles.detailsView}>
          ...
          <View style={styles.rightView}>
            ...
            <Text style={styles.boldText}>
              {formatValue(share.price * share.quantity)}
            </Text>
          </View>
        </View>
      </View>
