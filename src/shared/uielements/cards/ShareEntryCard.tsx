...
const ShareEntryCard = (props: ShareEntryCardProps) => {
  ...
  const typeColor =
    entry.type === OrderType.SELL ? colors.brown : colors.textSuccess;
  const allowed =
    (entry.type === OrderType.SELL &&
      ownedShare &&
      entry.quantity < ownedShare?.quantity) ||
    ...
    <View style={styles.mainView}>
      <View style={styles.detailsView}>
        <View style={styles.gridView}>
          <Text h3>
            {entry.type === OrderType.SELL ? "BID price" : "ASK price"}
          </Text>
