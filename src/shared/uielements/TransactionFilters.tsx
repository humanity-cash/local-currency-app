...
import { TxFilters, TxFilterStoreActions, TxFilterStoreReducer } from 'src/utils/types';
...
const getFilters = ({ onClear, store }: { onClear: any, store: string }): FiltersModule => {
    const [{
        ...
        isStartDate,
        endDate,
        isEndDate }, dispatch] = useStore<TxFilters, TxFilterStoreReducer>(store);

    const onStartDateChange = (selectedDate?: Date) => {
        const date = selectedDate
        if (date && endDate) {
            if (moment(date).isAfter(endDate)) {
                dispatch({ type: TxFilterStoreActions.UpdateEndDate, payload: { endDate: date } })
            }
        }
        dispatch({ type: TxFilterStoreActions.UpdateStartDate, payload: { startDate: selectedDate || startDate || undefined } })
    };
    ...
    const onEndDateChange = (selectedDate?: Date) => {
        ...
        if (date && startDate) {
            if (moment(startDate).isAfter(date)) {
                dispatch({ type: TxFilterStoreActions.UpdateStartDate, payload: { startDate: date } })
            }
        }
        dispatch({ type: TxFilterStoreActions.UpdateEndDate, payload: { endDate: selectedDate || startDate || undefined } })
    };
    ...
    const onTypeSelect = (selectedItem: string) => {
        dispatch({ type: TxFilterStoreActions.UpdateType, payload: { type: selectedItem } })
    }
