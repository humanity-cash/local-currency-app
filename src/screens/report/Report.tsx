            ...
            <ScrollView style={wrappingContainerBase}>
                <View style={styles.container}>
                    <View style={underlineHeaderB}>
                        <Text style={styles.headerText}>{Translation.REPORT.MAKE_REPORT}</Text>
                    </View>
                    <View style={styles.content}>
                        <View style={styles.bodyView}>
                        <Text style={styles.bodyText}>{Translation.REPORT.SELECT_TIME_TYPE}</Text>
                        </View>

                        <Text style={styles.text}>{Translation.LABEL.TIME_PERIOD}</Text>
                        <View style={styles.defaultAmountView}>
                            <TouchableOpacity 
                                style={reportType === ReportType.TODAY ? styles.selectedAmountItem : styles.defaultAmountItem} 
                                onPress={()=>setReportType(ReportType.TODAY)}
                            >
                                <Text style={reportType == ReportType.TODAY ? styles.selectedAmountText : styles.amountText}>Today</Text>
                            </TouchableOpacity>
                            <TouchableOpacity 
                                style={reportType === ReportType.WEEK ? styles.selectedAmountItem : styles.defaultAmountItem}  
                                onPress={()=>setReportType(ReportType.WEEK)}
                            >
                                <Text style={reportType === ReportType.WEEK ? styles.selectedAmountText : styles.amountText}>Week</Text>
                            </TouchableOpacity>
                            <TouchableOpacity 
                                style={reportType === ReportType.MONTH ? styles.selectedAmountItem : styles.defaultAmountItem} 
                                onPress={()=>setReportType(ReportType.MONTH)}
                            >
                                <Text style={reportType === ReportType.MONTH ? styles.selectedAmountText : styles.amountText}>Month</Text>
                            </TouchableOpacity>
                        </View>
                        ...
                        {userType === UserType.MERCHANT && <View>
                            ...
                            <View style={styles.typeView}>
                                ...
                                >
                                    <Picker.Item label="All" value="" style={styles.amountText}/>
                                    {merchantTransactions.map((u:string) => 
                                        <Picker.Item label={u} value={u} key={u} style={styles.amountText} />)
                                    }
                                </Picker>
                            </View>
                        </View>}
                    </View>
                    <Text style={styles.specifySearch}>{Translation.REPORT.SPECIFY_TIME_PERIOD}</Text>
                </View>
            </ScrollView>
