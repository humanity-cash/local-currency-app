...
import { PersonalDetailsErrors } from "../../utils/types";
...
export const PersonalDetailsForm = (props: PersonalDetailsProps) => {
	...
	const onValueChange = (name: any, change: any) => {
		...
		if (name === 'day' || name === 'month' || name === 'year') {
			updatePersonalDetails({ dateOfBirth: { ...personalDetails.dateOfBirth, [name]: change } });
			return;
		}
	...
	return (
		<View>
			...
			{showValidation && validationErrors.firstname && (<Text h3 style={{ marginTop: 5, color: colors.textError}}>{validationErrors.firstname}</Text>)}
			<BlockInput
				name="firstname"
			...
			{showValidation && validationErrors.lastname && (<Text h3 style={{ marginTop: 5, color: colors.textError}}>{validationErrors.lastname}</Text>)}
			<BlockInput
				name="lastname"
			...
			{showValidation && validationErrors.nationality && (<Text h3 style={{ marginTop: 5, color: colors.textError}}>{validationErrors.nationality}</Text>)}
			<SelectModal
				name="nationality"
			...
			{showValidation && validationErrors.dateOfBirth && (<Text h3 style={{ marginTop: 5, color: colors.textError}}>{validationErrors.dateOfBirth}</Text>)}
			<View style={{
				flexDirection: "row",
				width: '60%'
			}}>
